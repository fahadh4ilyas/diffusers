...
class Flux2KleinPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    ...
    def generate_image_latents(
        ...
        return image_latents, image_latent_ids, height, width
    ...
    def __call__(
        ...
        Args:
            ...
            image_latents (`torch.Tensor`, *optional*):
                Pre-generated image latents to be used as reference for image generation. If not provided, image
                latents will be generated by encoding the `image` input argument.
            image_latent_ids (`torch.Tensor`, *optional*):
                Pre-generated image latent position ids to be used as reference for image generation. If not provided,
                image latent ids will be generated corresponding to the `image` input argument.
        ...
        # 4. process images
        if image is not None and image_latents is None and image_latent_ids is None:
            image_latents, image_latent_ids, height, width = self.generate_image_latents(
                image=image,
